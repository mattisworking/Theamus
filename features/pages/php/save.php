...
if ($link_query == false) {
    echo alert_notify('admin', 'failure', 'There was an querying the link database.');
    die();
}
    ...
    if ($query != false) {
        if ($tData->count_rows($link_query) > 0) {
            ...
            if ($update_link == false) {
                alert_notify('danger', 'The links related to this page could not be updated.');
            } else {
                alert_notify('success', 'Links related to this page have been updated.');
            }
        }

        alert_notify('success', 'The changes to this page have been saved.');
    } else {
        alert_notify('failure', 'There was an issue saving this information.');
    }
}