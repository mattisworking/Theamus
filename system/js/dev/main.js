...
function add_js_file(source, force) {
    ...
    for (var i = 0; i < $("script").length; i++) {
        ...
        if (script_source === new_source) {
            if (!force) load_new = false;
            else ($("script")[i]).remove();
        }
    }
    
    if (load_new === true) {
        // This source is now "loading"
        ...
        // Add the loaded event and set the source to actually load the script
        script.onload = function() { loading[source] = "loaded"; };
        script.src = source;
    }
}
...
function add_css(source, force) {
    ...
    if (sources.indexOf(base+source) === -1) {
        ...
        if (load_new === true) {
            ...
            // Add the loaded event and set the source to actually load the script
            link.onload = function() { loading[source] = "loaded"; };
